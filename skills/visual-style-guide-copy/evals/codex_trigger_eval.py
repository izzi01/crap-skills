...
def judge_prompt(query: str, description: str) -> dict:
    prompt = f'''You are evaluating skill triggering for a coding/design assistant.\n\nSkill name: visual-style-guide-copy\nSkill description: {description}\n\nUser prompt:\n{query}\n\nTask:\nDecide whether this prompt SHOULD trigger the visual-style-guide-copy skill.\nThe skill is for writing or rewriting visual style guides and brand guideline documents: logo rules, color systems, typography, imagery guidance, layout guidance, brand voice/tone sections, applications, and do/don't rules.\n\nReturn JSON only with this exact shape:\n{{"should_trigger": true, "confidence": 0.0, "reason": "..."}}\n\nRules:\n- should_trigger must be true or false\n- confidence must be between 0 and 1\n- reason must be one sentence\n- If the prompt is landing-page copywriting, implementation-only, design critique only, token extraction, or grammar-only proofreading, usually return false.\n- If the prompt clearly asks to create or rewrite a brand/style-guide document or its sections, usually return true.'''
